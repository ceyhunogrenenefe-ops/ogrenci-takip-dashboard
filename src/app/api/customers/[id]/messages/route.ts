...
import { bumpStats } from "@/lib/automation";
...
export async function POST(req: NextRequest, { params }: Ctx) {
  ...
  await bumpStats(session.id, { totalCalls: 1, reached: 1 });

  let whatsapp: unknown = null;
  if (parsed.data.sendWhatsApp && customer.phone) {
    try {
      whatsapp = await sendWhatsAppText(customer.phone, parsed.data.content);
    } catch (e) {
